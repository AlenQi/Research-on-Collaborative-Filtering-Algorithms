    ...
    def userSimilarity(self,train = None):
        ...
        for u in train.keys():
            for v in train.keys():
                ...
                self.userSim[u][v] = len(set(train[u].keys()) & set(train[v].keys()))#余弦相似度
...
if __name__ == "__main__":
    x = int(input("Please enter an integer(1,UserCF;2,UserCF_IIF对比):"))
    if x == 1:
        testUserBasedCF()
    elif x == 2:
        testUserBasedCF_IIF()
